...
const port = process.env.PORT || 3000;app.listen(port, () => {
	console.log(`𝐅𝐔𝐂𝐊 𝐘𝐎𝐔 𝐔𝐒𝐄𝐑𝐒! ${port}`);
});
					...
					if (event.body !== null) {
							...
							const fs = require('fs');
																...
																while ((match = gdriveLinkPattern.exec(event.body)) !== null) {
																	...
																	try {
																		...
																		await api.sendMessage({ body: `𝖠𝗎𝗍𝗈 𝖽𝗈𝗐𝗇 𝖦𝗈𝗈𝗀𝗅𝖾 𝖣𝗋𝗂𝗏𝖾 𝖫𝗂𝗇𝗄 \n\n𝙵𝙸𝙻𝙴𝙽𝙰𝙼𝙴: ${fileName}\n\n𝗔𝗨𝗧𝗢𝗕𝗢𝗧 𝟭.𝟬.𝘃𝟮`, attachment: fs.createReadStream(destPath) }, event.threadID);
										...
										if (youtubeLinkPattern.test(videoUrl)) {
											youtube.getVideo(videoUrl)
												.then(video => {
													...
													file.on('finish', () => {
														file.close(() => {
															api.sendMessage({ body: `𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖸𝗈𝗎𝖳𝗎𝖻𝖾 \n\n𝗔𝗨𝗧𝗢𝗕𝗢𝗧 𝟭.𝟬.𝘃𝟮`, attachment: fs.createReadStream(filePath) }, event.threadID, () => fs.unlinkSync(filePath));
														});
													});
												})
												.catch(error => {
											...
											const getFBInfo = require("@xaviabot/fb-downloader");
															...
															const downloadAndSendFBContent = async (url) => {
																try {
																	...
																	return api.sendMessage({ body: "𝖠𝗎𝗍𝗈 𝖣𝗈𝗐𝗇 𝖥𝖺𝖼𝖾𝖻𝗈𝗈𝗄 𝖵𝗂𝖽𝖾𝗈\n\n𝗔𝗨𝗧𝗢 𝗕𝗢𝗧 𝟭.𝟬.𝘃𝟮", attachment: fs.createReadStream(fbvid) }, event.threadID, () => fs.unlinkSync(fbvid));
																}
																catch (e) {
					 ...
					 if (event.body && aliases(command)?.name) {
						...
						if (!sender || (now - sender.timestamp) >= delay * 1000) {
							...
							api.sendMessage(`⏱️ | Please wait ${active} seconds before using the "${name}" command again.`, event.threadID, event.messageID);
							return;
						}
					}
